...
- `netstandard2.0` in the official .NET 6 runtime container, using the package's
  `System.Text.Json 8` dependency graph.
...
dotnet restore eng/PackageSmoke/Telegram.BotAPI.PackageSmoke.csproj -p:RestoreAdditionalProjectSources=artifacts
...
The .NET 6 build intentionally leaves the runtime's end-of-support warning
visible. Passing the smoke run proves the tested fallback scenario; it does not
override the support policy declared by .NET.
